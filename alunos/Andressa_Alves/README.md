# Document Heading

👩‍💻 Andressa Alves de Souza

Olá! Seja bem-vindo(a) ao meu espaço no repositório da turma de Visualização de Dados e BI - SESI-SENAI 🚀

Sou apaixonada por tecnologia, análise de dados e desenvolvimento front-end. Atualmente estou estudando Python, Power BI, programação e análise de dados, buscando evoluir constantemente minhas habilidades na área de tecnologia.

📚 Sobre mim
🎓 Estudante do curso Visualização de Dados e BI - SESI-SENAI
💻 Desenvolvedora Front-End
📊 Estudando Análise de Dados e Business Intelligence
🐍 Aprendendo Python e ferramentas de dados
🚀 Interessada em IA Generativa e Tecnologia
📖 Gosto de estudar e aprender coisas novas

🛠️ Tecnologias e Ferramentas
Python
Power BI
Git e GitHub
HTML5
CSS3
JavaScript
Looker Studio
Excel
📌 Objetivos

Neste curso, pretendo:

Aprender mais sobre visualização de dados
Criar dashboards profissionais
Melhorar minhas habilidades em análise de dados
Trabalhar com projetos reais
Evoluir na área de tecnologia e BI

📂 Estrutura da minha pasta
Aqui você encontrará:

Exercícios
Projetos
Dashboards
Anotações
Desafios do curso

🌎 Contato

📧 Email: <andressanikki@gmail.com>

💼 GitHub: <https://github.com/andressasouza98>

✨ “A tecnologia move o mundo, mas o conhecimento transforma pessoas.” ✨

Aluna da turma de Visualização de Dados — Lab365 / SENAI SC

---

## Entregas

| Pasta | Conteúdo |

| `04.Exercicios/` | `pratica.py`, `cadastro_produtos.py` |
| `semana_04/` | `base_rh.csv`, `base_rh.ipynb`, `base_rh.py` |
| `projetos/` | `projeto_01.py` |
